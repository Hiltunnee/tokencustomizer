...
//Style
import { svgStyle, numberTokenStyle, textTokenStyle, smallTextTokenStyle, svgStyleNoInteraction, bigNumberTokenStyle } from "./styles";
...
export default function Token({text, color, borderColor, isNumberToken, onClick, interactable=true}) {
  ...
  const bigNumber = isNumberToken && text.length > 5;
  ...
  return (
    <Box component="svg" viewBox=" 8 20 84 61" sx={interactable ? svgStyle : svgStyleNoInteraction} onClick={onClick}>
      ...
      {!twoRows && (
        <text x="50" y="53" textAnchor="middle" dominantBaseline="middle" fill={borderColor} style={isNumberToken ? (bigNumber ? bigNumberTokenStyle : numberTokenStyle) : smallerText ? smallTextTokenStyle : textTokenStyle}>
          {text}
        </text>
      )}
      {twoRows && (
